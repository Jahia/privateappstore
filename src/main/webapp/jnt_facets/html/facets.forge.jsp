...
<query:definition var="mainQuery" scope="page">
    <query:selector
            nodeTypeName="${not empty wantedNodeType and not empty wantedNodeType.string ? wantedNodeType.string : 'nt:base'}"/>
    ...
    <query:equalTo value="true" propertyName="published"/>
</query:definition>
    ...
    <c:forEach items="${jcr:getNodes(currentNode, 'jnt:facet')}" var="facet">
        ...
        <c:if test="${not empty facetPropertyName}">
            <c:choose>
                ...
                <c:otherwise>
                    <fmt:message var="translatedFacetProperty"
                                 key="${fn:replace(facetNodeTypeName,':','_')}.${fn:replace(facetPropertyName,':','_')}"/>
        ...
        <c:choose>
            <c:when test="${jcr:isNodeType(facet, 'jnt:fieldFacet') or jcr:isNodeType(facet, 'jnt:fieldHierarchicalFacet') or jcr:isNodeType(facet, 'jnt:dateFacet')}">
                <c:choose>
                    ...
                    <c:otherwise>
                        ...
                        <c:if test="${not empty currentFacetValueRenderer.string}">
                            <c:set target="${facetValueRenderers}" property="${facetPropertyName}"
                                   value="${currentFacetValueRenderer.string}"/>
                        </c:if>
                    </c:otherwise>
                </c:choose>
                <c:if test="${not empty currentField and not facet:isFacetApplied(facetPropertyName, activeFacetsVars[activeFacetMapVarName], facetNodeType.propertyDefinitionsAsMap[facetPropertyName])}">
                    ...
                    <c:if test="${not empty currentFacetValueRenderer.string}">
                        <c:set var="facetQuery"
                               value="${facetQuery}&${paramPrefix}labelRenderer=${currentFacetValueRenderer.string}"/>
                    </c:if>
                            ...
                            <c:when test="${functions:isIterable(facetPropValue)}">
                                <c:forEach items="${facetPropValue}" var="facetPropValueItem">
                                    <c:if test="${not empty facetPropValueItem.string}">
                                        <c:set var="facetQuery"
                                               value="${facetQuery}&${paramPrefix}${propertyDefinition.name}=${facetPropValueItem.string}"/>
                                    </c:if>
                                </c:forEach>
                            </c:when>
                            <c:otherwise>
                                <c:if test="${not empty facetPropValue.string}">
                                    ...
                                    <c:if test="${jcr:isNodeType(facet, 'jnt:fieldHierarchicalFacet') and propertyDefinition.name == 'prefix'}">
                                        <c:set var="currentActiveFacets"
                                               value="${activeFacetsVars[activeFacetMapVarName][facetPropertyName]}"/>
                                        <c:choose>
                                            <c:when test="${not empty currentActiveFacets}">
                                                <c:set var="paramValue"
                                                       value="${facet:getDrillDownPrefix(currentActiveFacets[fn:length(currentActiveFacets) - 1].key)}"/>
                                            </c:when>
                                            <c:otherwise>
                                                <c:set var="paramValue"
                                                       value="${facet:getIndexPrefixedPath(facetPropValue.string)}"/>
                                            </c:otherwise>
                                        </c:choose>
                                    </c:if>
                                    <c:set var="facetQuery"
                                           value="${facetQuery}&${paramPrefix}${propertyDefinition.name}=${paramValue}"/>
                                </c:if>
                                <query:equalTo value="true" propertyName="published"/> </c:otherwise>
                        </c:choose>
                    </c:forEach>
            ...
            <ul>
                ...
                <c:if test="${not facet:isFacetValueApplied(facetValue, activeFacetsVars[activeFacetMapVarName])}">
                    ...
                    <c:url var="facetUrl" value="${url.mainResource}">
                        ...
                        <c:param name="forgeModulesList" value="${functions:encodeUrlParam(facetDrillDownUrl)}"/>
                    </c:url>
